  ...
  try {
    ...
    if (data.status === 200 && data.cookie) {
      ...
      if (cookieMatch) {
        ...
        console.log("[DataDome Hook] ✅ Retrieved cookie:", cookieValue);
        return `datadome=${cookieValue}`;
      }
    }

    console.log("[DataDome Hook] ⚠️ Failed to retrieve cookie:", data);
    return null;
  } catch (error) {
...
function containsDatadomeCookie(headers) {
  return (
    headers?.Cookie?.includes("datadome=") ||
    headers?.cookie?.includes("datadome=")
  );
}
...
export const datadomeHook = async (options) => {
  if (options?.headers?.removedDatadomeCookie) {
    delete options.headers.Cookie;
    delete options.headers.cookie;
    delete options.headers.removedDatadomeCookie;
  }
  ...
  if (isCacheValid()) {
    console.log("[DataDome Hook] 📦 Using cached cookie");
    if (!options.headers) {
      options.headers = {};
    }
  ...
  console.log("[DataDome Hook] 🔄 Fetching new cookie...");
...
export const datadomeHookRetry = (error, retryCount) => {
  ...
  error.request.options.headers.removedDatadomeCookie = true;
