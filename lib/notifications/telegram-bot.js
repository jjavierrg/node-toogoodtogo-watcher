...
const botCommands = [
  ...
  {
    command: "show_price_change",
    description: "Activate alert for price change.",
  },
  {
    command: "start",
...
export function createTelegramBot() {
  ...
  bot.command("show_price_change", showPriceChangeCommand);
...
Open the login email on your PC and paste the numeric code you received in the email.
...
async function loginContinueCommand(context) {
  ...
  const textParts = context.update.message.text.split(" ");
  const code = textParts.length > 1 ? textParts[1].trim() : null;

  if (!code) {
    context.reply(`To continue the login process run:
/login_continue <code>`);
    return;
  }

  const authPollingResponse = await authByRequestPin(
    cache.loginPollingId,
    code,
  );
...
Enabled : ${config.bot ? "✅" : "🚫"}
Activate alert for unchanged stock : ${config.messageFilter.showUnchanged ? "✅" : "🚫"
    }
Activate alert for decreasing stock : ${config.messageFilter.showDecrease ? "✅" : "🚫"
    }
Activate alert for sold out : ${config.messageFilter.showDecreaseToZero ? "✅" : "🚫"
    }
Activate alert for increasing stock : ${config.messageFilter.showIncrease ? "✅" : "🚫"
    }
Activate alert for new market available : ${config.messageFilter.showIncreaseFromZero ? "✅" : "🚫"
    }
Activate alert for price change : ${config.messageFilter.showPriceChange ? "✅" : "🚫"
    }
  `);
}
...
function showPriceChangeCommand(context) {
  const activate = setOption("showPriceChange");
  context.reply(
    (activate ? "✅" : "❌") +
    " Price change will" +
    (activate ? " " : " not ") +
...
function addOrEnableChat(chat) {
  ...
  const updatedChat = addOrUpdateChat({ ...chat, disabled: false });
