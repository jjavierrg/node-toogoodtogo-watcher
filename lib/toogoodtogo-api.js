...
import { datadomeHook, datadomeHookRetry } from "./hooks/datadomeHook.js";
import { correlationIdHook } from "./hooks/correlationIdHook.js";
...
const api = got.extend({
  ...
  hooks: {
    beforeRequest: [correlationIdHook, datadomeHook],
    beforeRetry: [datadomeHookRetry],
  },
  responseType: "json",
...
export function authByRequestPin(polling_id, pin) {
  return api
    .post("auth/v5/authByRequestPin", {
      headers: getHeaders(),
      json: {
        device_type: config.get("api.deviceType", "ANDROID"),
        email: config.get("api.credentials.email"),
